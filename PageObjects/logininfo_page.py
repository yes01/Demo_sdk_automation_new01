...
class LogininfoPage(BasePage):
     ...
     #SDK初始化_登录功能
    @allure.step("SDK授权初始化_登录功能_信息界面")
    def is_authorize_login_info(self):
        logger.info("-----------------------SDK授权初始化_登录功能_信息界面-----------------------")
        doc = "SDK授权初始化_登录功能_信息界面"
        ...
        time.sleep(2)
        self.Coordinate_positioning(loc.x_y_03)
        self.switch_scenes()

    # SDK初始化_登录功能
    @allure.step("SDK未授权初始化_登录功能_信息界面")
    def not_authorize_login_info(self):
        logger.info("-----------------------SDK未授权初始化_登录功能_信息界面-----------------------")
        doc = "SDK未授权初始化_登录功能_信息界面"
        self.click_element(loc.initialization_sdk_01, doc)
        self.wait_eleVisible(loc.initialization_sdk_02, doc=doc)
        self.click_element(loc.initialization_sdk_02, doc)
        self.wait_eleVisible(loc.initialization_sdk_02, doc=doc)
